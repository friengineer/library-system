...
void splitQuery(char * query) {
  ...
  while (word != NULL) {
    // gives the first word to searchTitle()
    searchTitle(word, 0);

    // // remember to return 0
    // while (searchTitle(&word) != 0) {
    //
    //   if (matchedBooks[k] == 0) {
    //
...
void displaySearch() {
  for (int i = 0; i < 100; i++) {
    if (matchedBooks[i] != 0) {
      printf("%i,%s,%s,%s\n",
             books[matchedBooks[i] - 1].id,
             books[matchedBooks[i] - 1].title,
             books[matchedBooks[i] - 1].authorFirstName,
             books[matchedBooks[i] - 1].authorSurname);
    }
  }
}
